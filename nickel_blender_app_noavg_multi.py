...
# --------------- Inventory & multi-cargo logic ---------------
def apply_allocation(norm_df, alloc_df):
    """Subtract allocation from inventory, return updated norm_df."""
...
def balance_multi_cargoes(norm_df, T, floor, x_min, prefer_min_lots=True, k=None, k_max_cap=50):
    """Build k cargoes (or auto-max if k is None), each Ni >= floor, returning list of (df, kpis, solver)."""
    results = []
    inv = norm_df.copy()
    if inv["WMT_available"].sum() < T:
        return results, inv  # no cargo possible
    count = 0
    while (k is None and count < k_max_cap) or (k is not None and count < k):
        ...
        if cur["WMT_available"].sum() < T:
            break
        rows_df, kpis, solver = optimize_single_cargo(cur, T, floor, None, x_min, prefer_min_lots)
...
def to_xlsx_sheets(sheets: dict, filename: str):
    ...
    bio.seek(0)
    st.download_button("Download XLSX", data=bio.getvalue(), file_name=filename,
...
if uploaded is not None:
    ...
    if {"LotID","WMT_available","Ni"}.issubset(norm_df.columns):
        ...
        results, leftover_inv = balance_multi_cargoes(norm_df, cargo_wmt, target_ni, x_min, prefer_min_lots, k=k)
        if not results:
            ...
            for i, (df_out, kpis, solver) in enumerate(results, start=1):
                ...
                st.dataframe(df_out)
                st.write({"Cargo_WMT": df_out["WMT_to_load"].sum(),
                          ...
                          "Solver": solver})
                st.download_button(f"Download Cargo {i} CSV",
            ...
            # Per-cargo leftovers + final leftover
